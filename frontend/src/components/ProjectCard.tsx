...
const pulseAnimation = `
  ...
  .ant-card:hover .card-action-buttons {
    opacity: 1 !important;
  }

  .project-card .card-cover-btn.ant-btn {
  ...
  .project-card .card-cover-btn.ant-btn .anticon {
    font-size: 12px !important;
    color: inherit !important;
  }

  .project-card .card-cover-btn.ant-btn:hover {
    background: #ffffff !important;
    color: #0e7c66 !important;
    transform: scale(1.06);
  }

  .project-card .card-cover-btn-danger.ant-btn:hover {
    background: #ffffff !important;
...
if (typeof document !== 'undefined') {
  let style = document.getElementById('project-card-hover-styles') as HTMLStyleElement | null
  if (!style) {
    style = document.createElement('style')
    style.id = 'project-card-hover-styles'
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onDelete, onRetry, onClick }) => {
  ...
  const getCategoryInfo = (category?: string) => {
    const categoryMap: Record<string, { name: string; icon: string; color: string }> = {
      'default': { name: '默认', icon: '🎬', color: '#0e7c66' },
      'knowledge': { name: '知识科普', icon: '📚', color: '#52c41a' },
      'business': { name: '商业财经', icon: '💼', color: '#faad14' },
      'opinion': { name: '观点评论', icon: '💭', color: '#3d5a80' },
      'experience': { name: '经验分享', icon: '🌟', color: '#2a9d8f' },
      'speech': { name: '演讲脱口秀', icon: '🎤', color: '#c9851a' },
  ...
  // 缩略图缓存管理（v2：提高分辨率后换 key，避免沿用旧糊图）
            ...
            await new Promise((resolve, reject) => {
              ...
              video.onseeked = () => {
                ...
                try {
                  ...
                  // 高清封面：按展示宽度约 3 倍截取，避免 Retina 下发糊
                  const maxWidth = 960
      ...
      cover={
        ...
        >
          ...
          <div style={{
            ...
            <span style={{ fontSize: '11px', color: 'rgba(255, 255, 255, 0.92)', fontWeight: 500, letterSpacing: '0.01em' }}>
              ...
              {normalizedStatus === 'failed' ? (
                <>
                  ...
                  >
                    <Button
                      type="text"
                      className="card-cover-btn card-cover-btn-danger"
                      icon={<DeleteOutlined />}
                      onClick={(e) => {
                        e.stopPropagation()
                      }}
          ...
          {(normalizedStatus === 'importing' || normalizedStatus === 'processing' || normalizedStatus === 'failed') ? (
            ...
            <div style={{ 
              ...
              <div style={{
                ...
                <div style={{ color: '#0e7c66', fontSize: '11px', fontWeight: 600, lineHeight: '12px' }}>
                  {project.total_clips || 0}
                </div>
                <div style={{ color: '#6b7585', fontSize: '8px', lineHeight: '9px' }}>
                  切片
                </div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ color: '#3d4654', fontSize: '11px', fontWeight: 600, lineHeight: '12px' }}>
