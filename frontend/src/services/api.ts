...
export const projectApi = {
  ...
  downloadVideo: async (projectId: string, clipId?: string, collectionId?: string) => {
    ...
    try {
      ...
      const response = await axios.get(apiUrl(url), { 
        responseType: 'blob',
        headers: {
          'Accept': 'application/octet-stream, application/zip, video/mp4'
        }
      })
      ...
      const isZip = contentType.includes('zip') || (!clipId && !collectionId)
      let filename = clipId ? `clip_${clipId}.mp4` : 
                     collectionId ? `collection_${collectionId}.mp4` : 
                     `project_${projectId}_clips.zip`
      ...
      const blob = new Blob([response.data], {
        type: isZip ? 'application/zip' : 'video/mp4'
      })
  ...
  getClipVideoUrl: (projectId: string, clipId: string, _clipTitle?: string): string => {
    // 使用projects路由获取切片视频
    return apiUrl(`/projects/${projectId}/clips/${clipId}`)
  },
