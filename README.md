...
- **抖音**：公开视频优先走分享页直链，**多数情况无需 Cookie**。请粘贴 `www.douyin.com/video/...` 或 App「分享 → 复制链接」短链（勿贴用户主页）。若仍失败再上传 `data/cookies/douyin.txt`。

本机模式还可直接选择已登录浏览器（对 YouTube 较有效；抖音更推荐分享页或 cookies.txt）。
