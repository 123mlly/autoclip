...
- ✂️ **按台词剪辑**: 用语音识别/字幕文稿定位，按句删除并重新剪辑切片
- 📤 **B站上传**【开发中】: 自动上传切片视频到B站
- 📱 **移动端支持**【开发中】: 响应式设计，正在完善移动端体验
- 🔐 **账号管理**: 支持B站多账号管理，账号健康检查
- 📊 **数据统计**: 完整的项目管理和数据统计功能
...
cp env.example .env
# 编辑 .env，填入 API_DASHSCOPE_API_KEY=sk-xxx

# Docker一键启动（生产：前后端同源 http://localhost:8000）
./docker-start.sh

# 开发环境启动（前端 :3000，后端 :8000）
./docker-start.sh dev
...
- **生产 Docker 应用界面**: [http://localhost:8000](http://localhost:8000)（前后端同源）
- **开发 Docker / 本地前端**: [http://localhost:3000](http://localhost:3000)
- **后端API**: [http://localhost:8000](http://localhost:8000)
- **API文档**: [http://localhost:8000/docs](http://localhost:8000/docs)
- **Flower监控**: `docker compose --profile monitoring up -d` 后访问 [http://localhost:5555](http://localhost:5555)
...
**Q: 按台词剪辑怎么用？**
A: 在切片预览中点击「按台词剪辑」。这里用的是语音识别/字幕文件生成的文稿时间轴（不是画面上的烧录字幕），可按句删除对应画面并重新导出：
