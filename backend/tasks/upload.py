    ...
    try:
        ...
        if clip_title:
            ...
            possible_paths.extend([
                ...
                project_output_dir / "clips" / f"{clip_id}_{clean_title}.mp4",
            ])
        ...
        if not video_path:
            ...
            if clips_dir.exists():
                ...
                # 优先按 clip_id 前缀匹配（流水线常见命名：{id}_{title}.mp4 或 {n}_{title}.mp4）
                for mp4_file in mp4_files:
                    if mp4_file.name.startswith(f"{clip_id}_") or clip_id in mp4_file.name:
                        video_path = mp4_file
                        logger.info(f"通过clip_id匹配找到: {video_path}")
                        break

                if not video_path and clip_title:
                    for mp4_file in mp4_files:
                        if any(keyword and keyword in mp4_file.name for keyword in clip_title.split()[:3]):
                            video_path = mp4_file
                            logger.info(f"通过标题匹配找到: {video_path}")
                            break

                if not video_path and len(mp4_files) == 1:
        ...
        if success:
            ...
            # upload_clip_sync 已写入 completed + bv_id，这里仅兜底
...
def _resolve_clip_video_path(db, project_id: str, clip_id: str):
    """根据 clip_id 在项目 output/clips 下定位视频文件。"""
    from ..models.clip import Clip
    import re

    project_output_dir = get_project_output_directory(str(project_id))
    clip = db.query(Clip).filter(Clip.id == clip_id).first()
    if not clip:
        raise ValueError(f"切片记录不存在: {clip_id}")
    ...
    if clips_dir.exists():
        ...
        if clip_title:
            for mp4_file in mp4_files:
                if any(k and k in mp4_file.name for k in clip_title.split()[:3]):
                    return mp4_file
        if len(mp4_files) == 1:
            return mp4_files[0]
...
@celery_app.task(bind=True, name='backend.tasks.upload.upload_youtube_clip_task')
def upload_youtube_clip_task(self, record_id: str, clip_id: str):
    """上传切片到 YouTube"""
    ...
    record_id_int = None
    try:
        logger.info(f"开始上传到 YouTube: record_id={record_id}, clip_id={clip_id}")
        upload_service = YouTubeUploadService(db)
        record_id_int = int(record_id)

        record = upload_service.get_record(record_id_int)
        if not record:
            raise ValueError(f"YouTube 投稿记录不存在: {record_id}")

        video_path = _resolve_clip_video_path(db, str(record.project_id), clip_id)
        if video_path.stat().st_size == 0:
            raise ValueError("视频文件为空")

        success = upload_service.upload_clip_sync(record_id_int, str(video_path))
        if success:
            logger.info(f"YouTube 上传成功: {clip_id}")
            upload_service.update_status(record_id_int, "completed")
        else:
            record = upload_service.get_record(record_id_int)
            err = (record.error_message if record else None) or "上传失败"
            logger.error(f"YouTube 上传失败: {err}")
            upload_service.update_status(record_id_int, "failed", err)
    except Exception as e:
        logger.error(f"YouTube 上传任务失败: {e}")
        if record_id_int is not None:
            try:
                upload_service = YouTubeUploadService(db)
                upload_service.update_status(record_id_int, "failed", str(e))
            except Exception:
                pass
        raise
    finally:
