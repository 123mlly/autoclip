...
def _parse_account_id(account_id) -> Optional[int]:
    """将账号 ID 统一转为 int。"""
    try:
...
def _build_upload_metadata(record: UploadRecord) -> Dict[str, Any]:
    """从投稿记录构建上传元数据。"""
    tags: Any = record.tags or ""
    ...
    return {
        "title": record.title,
        "desc": record.description or record.title or "",
        "description": record.description or record.title or "",
        "tid": record.partition_id or 21,
        "partition_id": record.partition_id or 21,
        "tag": tags,
        "tags": tags,
        "source": "",
        "copyright": 1,
...
class BilibiliAccountService:
    ...
    def get_account(self, account_id) -> Optional[BilibiliAccount]:
        """获取指定账号（兼容 int / str ID）"""
        parsed = _parse_account_id(account_id)
        if parsed is None:
            return None
        return self.db.query(BilibiliAccount).filter(BilibiliAccount.id == parsed).first()
    ...
    def delete_account(self, account_id) -> bool:
        ...
        try:
            ...
            upload_records = self.db.query(UploadRecord).filter(UploadRecord.account_id == account.id).all()
    ...
    def check_account_status(self, account_id) -> bool:
        """检查账号状态"""
        account = self.get_account(account_id)
        if not account:
            return False
...
class BilibiliUploadService:
    ...
    def create_upload_record(self, project_id: UUID, upload_data: UploadRequest) -> UploadRecord:
        ...
        if not account:
            raise ValueError("账号不存在，请先在设置页导入 B 站 Cookie")
        ...
        record = UploadRecord(
            ...
            tags=json.dumps(upload_data.tags or []),
    ...
    async def upload_clip(self, record_id: int, video_path: str, max_retries: int = 3) -> bool:
        """上传单个切片"""
        return self.upload_clip_sync(record_id, video_path, max_retries=max_retries)
    
    def update_upload_status(self, record_id, status: str, error_message: str = None) -> bool:
        """更新投稿状态"""
        try:
            record = self.db.query(UploadRecord).filter(UploadRecord.id == int(record_id)).first()
            if not record:
                return False
    ...
    def get_upload_record(self, record_id) -> Optional[UploadRecord]:
        """获取指定投稿记录（兼容 int / str ID）"""
        try:
            return self.db.query(UploadRecord).filter(UploadRecord.id == int(record_id)).first()
        except (TypeError, ValueError):
            return None
    ...
    def upload_clip_sync(self, record_id: int, video_path: str, max_retries: int = 3) -> bool:
        ...
        try:
            ...
            if not cookies:
                ...
                record.status = "failed"
                record.error_message = "Cookie 解密失败，请重新导入账号"
                self.db.commit()
                return False
            
            uploader = BilibiliUploader(cookies)
            success = uploader.upload_video(
                video_path=video_path,
                metadata=_build_upload_metadata(record),
