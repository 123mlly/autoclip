...
async def test_speech_recognition(request: SpeechRecognitionRequest):
    ...
    try:
        ...
        elif request.method == "sensevoice":
            # 允许别名；具体归一在识别器内完成
            if not request.model:
                raise HTTPException(status_code=400, detail="SenseVoice 模型不能为空")
        elif request.method == "faster_whisper":
            allowed = {"tiny", "base", "small", "medium", "large", "large-v2", "large-v3", "distil-large-v3"}
            if request.model not in allowed:
                raise HTTPException(
                    status_code=400,
                    detail=f"不支持的 faster-whisper 模型: {request.model}"
                )
...
async def get_install_guide(method: str = Query(..., description="语音识别方法")):
    ...
    guides = {
        ...
        "faster_whisper": {
            "title": "faster-whisper 安装指南",
            "description": "CTranslate2 版 Whisper：更省内存，段级时间戳更稳，适合切片",
            "steps": [
                "1. 安装依赖: pip install faster-whisper",
                "2. 安装 ffmpeg",
                "3. 在 .env 中设置:",
                "   SPEECH_RECOGNITION_METHOD=faster_whisper",
                "   SPEECH_RECOGNITION_MODEL=base",
                "   SPEECH_RECOGNITION_LANGUAGE=auto",
                "4. 可选: FASTER_WHISPER_COMPUTE_TYPE=int8（CPU）或 float16（GPU）",
                "5. 重启后端与 Celery worker"
            ],
            "notes": [
                "推荐模型: tiny / base / small（内存紧用 tiny/base）",
                "首次运行会下载对应 Whisper 权重",
                "输出带段级时间戳，适合 AutoClip 切片"
            ]
        },
        "sensevoice": {
            "title": "SenseVoice（FunASR）安装指南",
            "description": "安装本地 SenseVoice，支持中/英/日/韩/粤等多语转写，通常比 Whisper 更省内存",
            "steps": [
                "1. 安装依赖: pip install funasr",
                "2. 安装 ffmpeg（macOS: brew install ffmpeg）",
                "3. 在 .env 中设置:",
                "   SPEECH_RECOGNITION_METHOD=sensevoice",
                "   SPEECH_RECOGNITION_MODEL=iic/SenseVoiceSmall",
                "   SPEECH_RECOGNITION_LANGUAGE=auto",
                "4. 可选设备: SENSEVOICE_DEVICE=cpu|cuda|auto",
                "5. 重启后端与 Celery worker"
            ],
            "notes": [
                "首次运行会从 ModelScope 下载 SenseVoiceSmall 与 VAD 模型",
                "推荐模型: iic/SenseVoiceSmall",
                "语言可用 auto / zh / en / ja / ko / yue"
            ]
        },
        "openai_api": {
