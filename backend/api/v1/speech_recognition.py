...
async def test_speech_recognition(request: SpeechRecognitionRequest):
    ...
    try:
        ...
        if request.method == "whisper_local":
            ...
            raise HTTPException(
                status_code=400,
                detail="SenseVoice（funasr）已移除，请改用 faster_whisper"
            )
...
async def get_install_guide(method: str = Query(..., description="语音识别方法")):
    ...
    guides = {
        ...
        "sensevoice": {
            "title": "SenseVoice 已移除",
            "description": "FunASR / SenseVoice 已从默认依赖中移除，请使用 faster-whisper",
            "steps": [
                "1. 设置 SPEECH_RECOGNITION_METHOD=faster_whisper",
                "2. 设置 SPEECH_RECOGNITION_MODEL=base（或 tiny/small）",
                "3. 重启后端与 Celery worker"
            ],
            "notes": [
                "Docker 镜像不再安装 funasr，以加快构建、减小体积"
            ]
        },
        "openai_api": {
