...
def _resolve_srt_file(project_dir: Path) -> Optional[Path]:
    """查找项目字幕文件：优先 raw/input.srt，其次 metadata/input.srt，再回退到目录内其他 .srt。"""
    candidates = [
    ...
    for directory in (project_dir / "raw", project_dir / "metadata"):
        if not directory.exists():
            continue
        for srt in sorted(directory.glob("*.srt")):
            # 跳过分块/编辑中间产物
            if srt.name.startswith("step") or "_edited" in srt.name:
                continue
            return srt
    return None
...
async def get_clip_subtitles(
    ...
    try:
        ...
        # 查找原始SRT文件（ASR 可能写在 metadata/，上传字幕在 raw/）
        projects_dir = get_projects_directory()
        project_dir = projects_dir / project_id
        srt_file = _resolve_srt_file(project_dir)
