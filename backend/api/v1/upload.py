...
async def create_upload_task(
    ...
    """创建投稿任务"""
    try:
        if not upload_data.clip_ids:
            raise HTTPException(status_code=400, detail="请至少选择一个切片")
        if not upload_data.title or not upload_data.title.strip():
            raise HTTPException(status_code=400, detail="请填写视频标题")

        record = upload_service.create_upload_record(project_id, upload_data)
        
        # 启动异步上传任务（每个切片一个任务；当前实现按记录投稿一次）
        primary_clip_id = upload_data.clip_ids[0]
        upload_clip_task.delay(str(record.id), primary_clip_id)
        ...
        raise HTTPException(status_code=500, detail=f"创建投稿任务失败: {str(e)}")
