    ...
    try:
        ...
        if collection_id:
            ...
            for clip in clips:
                ...
                file_path = Path(clip.video_path)
                if file_path.exists():
                    safe_name = VideoProcessor.sanitize_filename(
                        clip.title or f"clip_{clip.id}"
                    )
                    clip_files.append((file_path, f"{safe_name}.mp4"))

            if not clip_files:
                raise HTTPException(status_code=404, detail="没有可下载的切片视频")

            buffer = io.BytesIO()
            with zipfile.ZipFile(buffer, "w", zipfile.ZIP_STORED) as zf:
                used_names: set[str] = set()
                for file_path, name in clip_files:
                    final_name = name
                    if final_name in used_names:
                        stem = Path(name).stem
                        final_name = f"{stem}_{file_path.stem[:8]}.mp4"
                    used_names.add(final_name)
                    zf.write(file_path, final_name)

            buffer.seek(0)
            project_name = VideoProcessor.sanitize_filename(project.name or project_id)
            zip_filename = f"{project_name}_clips.zip"
