...
| 文件导入 | 上传本地视频，可选附带 SRT；无字幕时用本地 **Whisper** 自动识别 |
...
- **媒体**: FFmpeg、yt-dlp、**openai-whisper**（无字幕时本地语音识别）
...
- `pip install -r requirements.txt` 会安装 `openai-whisper`（含 PyTorch，体积较大）

### 硬件推荐

无字幕时会跑本地 Whisper，对 CPU / 内存要求明显高于「只跑 API」：

| 场景 | CPU | 内存 | 磁盘 | 说明 |
|------|-----|------|------|------|
| 最低可用 | 4 核 | 8 GB | 20 GB+ | 短视频 + Whisper `tiny`/`base`；Docker 首次构建与模型下载较慢 |
| **推荐** | 8 核+ | **16 GB** | 40 GB+ | 日常切片；默认 `base`/`small` 更顺畅 |
| 更舒适 | 8 核+ / Apple Silicon | 32 GB | 60 GB+ | 长视频、`small`/`medium`；有 NVIDIA GPU 时 Whisper 明显更快 |

补充：

- **磁盘**：Docker 镜像含 PyTorch，另加 Whisper 模型缓存与 `data/` 视频文件，建议预留充足空间  
...
首次会**重新构建镜像并打包前端**，可能需要较长时间（镜像含 Whisper / PyTorch，首次下载依赖较大）。成功后浏览器打开：
...
# 可选：NVIDIA GPU 加速 Whisper（无显卡勿用，会启动失败）
./docker-start.sh gpu
...
pip install -r requirements.txt   # 含 openai-whisper，首次较慢
...
**Redis 用途**：Celery 任务队列（导入、Whisper、切片流水线等）与处理进度缓存。业务数据在 SQLite（`data/`），不在 Redis。
...
├── Dockerfile               # CPU 生产镜像
├── Dockerfile.gpu           # CUDA / Whisper worker 镜像
...
**无字幕 / 语音识别？**  
未附带 SRT 时使用本地 **Whisper**（`openai-whisper`）生成字幕。Docker 默认镜像为 CPU；首次运行会下载模型（如 `base` / `small`）。有 NVIDIA 时用 `./docker-start.sh gpu`，Whisper 会在 `cuda` 可用时自动走 GPU（也可用环境变量 `WHISPER_DEVICE` 强制 `cpu`/`cuda`）。
