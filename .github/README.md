...
| 能力 | 说明 |
|------|------|
| 链接导入 | 粘贴 B站 / YouTube / 抖音链接，自动下载并创建项目 |
...
- **媒体**: FFmpeg、yt-dlp、**faster-whisper**（默认本地 ASR；可选 openai-whisper）
...
- `pip install -r requirements.txt` 会安装 `faster-whisper`（默认）与 `openai-whisper` 等（体积较大）
...
- **GPU（可选）**：非必须；无 GPU 时用 CPU 即可，长视频识别会慢一些  
  - **默认 Docker 为 CPU**：`./docker-start.sh`（Windows：`docker-start.bat`）  
  - 有 NVIDIA 时再用：`./docker-start.sh gpu` / `docker-start.bat gpu`（叠加 `docker-compose.gpu.yml`，仅 worker 用 CUDA 镜像；需驱动 + [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html)）  
- **仅附带 SRT、不做语音识别**：8 GB 内存通常也能跑通导入与 AI 流水线  
- **切换 ASR**：在 `.env` 设置 `SPEECH_RECOGNITION_METHOD=faster_whisper|whisper_local`（见 `env.example`）  
...
| Windows（CMD） | `docker-start.bat`（需 [Docker Desktop](https://www.docker.com/products/docker-desktop/) 已安装并启动） |
...
> 一键启动脚本若发现没有 `.env`，会自动从 `env.example` 复制一份。
...
> 说明：启动脚本已包含前端。构建时会执行 `npm run build`，运行时在 **8000** 端口同时提供网页和接口。只有 `dev` 模式才会单独在 **3000** 跑 Vite 开发服务器。  
> Docker 内 `REDIS_URL` 自动指向服务名 `redis`（`redis://redis:6379/0`），无需改 `.env` 里的 localhost。

**4. 常用命令**

| 操作 | macOS / Linux / WSL | Windows CMD |
|------|---------------------|-------------|
| 查看状态 | `./docker-status.sh` | `docker-status.bat` |
| 停止服务 | `./docker-stop.sh` | `docker-stop.bat` |
| 看日志 | `docker compose logs -f` | 同左 |
| GPU 启动 | `./docker-start.sh gpu` | `docker-start.bat gpu` |
| 开发模式 | `./docker-start.sh dev` | `docker-start.bat dev` |

```bash
# GPU 自检（有 NVIDIA 时）
docker compose -f docker-compose.yml -f docker-compose.gpu.yml exec celery-worker \
  python3 -c "import torch; print(torch.cuda.is_available())"
```

> 无显卡请勿使用 `gpu` 模式，容器可能启动失败。
...
├── docker-start.sh / .bat   # Docker 一键启动（Unix / Windows CMD）
├── docker-stop.sh / .bat    # 停止服务
├── docker-status.sh / .bat  # 查看状态
...
**AI 不工作？**  
检查 `.env` 或设置页中的 API Key；Docker 需保证 `env_file: .env` 已加载（`./docker-start.sh` / `docker-start.bat` 会提示）。

**Windows 上怎么跑 Docker？**  
安装并启动 Docker Desktop 后，在项目目录用 **CMD** 执行 `docker-start.bat`（不要依赖 `.sh`）。停止 / 状态：`docker-stop.bat`、`docker-status.bat`。也可在 WSL2 里继续用 `./docker-start.sh`。
...
Docker 默认镜像为 CPU；首次运行会下载模型。有 NVIDIA 时用 `./docker-start.sh gpu` 或 `docker-start.bat gpu`；可用 `WHISPER_DEVICE` / `FASTER_WHISPER_DEVICE` 强制 `cpu`/`cuda`，CPU 上可用 `FASTER_WHISPER_COMPUTE_TYPE=int8`。
...
**YouTube / 抖音下载失败？**  
yt-dlp 常需登录 Cookie。**Docker 内无法读取本机浏览器**：在链接导入页上传从已登录站点导出的 `cookies.txt`（扩展如 Get cookies.txt LOCALLY）：

- YouTube → `youtube.com` → 保存为 `data/cookies/youtube.txt`
- 抖音 → `douyin.com`（支持 `v.douyin.com` 短链）→ 保存为 `data/cookies/douyin.txt`

本机模式还可直接选择已登录浏览器。Cookie 过期后需重新导出上传。
