...
- Docker 20.10+
- Docker Compose 2.0+
- **推荐 16GB 可用内存**（最低约 8GB；无字幕需跑 Whisper）
- **推荐 40GB+ 可用磁盘**（镜像含 PyTorch，另加模型与视频数据）
...
- **生产应用界面**: http://localhost:8000（前后端同源）
- **开发前端**: http://localhost:3000（`./docker-start.sh dev`）
- **后端API**: http://localhost:8000/api/v1
- **API文档**: http://localhost:8000/docs
- **Flower监控**: `docker compose --profile monitoring up -d` 后访问 http://localhost:5555
